...
def create_node_headers(dirname, filename):
    ...
    return {
        "node": node,
        "metadataHeader": csv_header,
        "importHeader": import_header,
        "metadataPath": os.path.join(dirname, filename),
    }
...
def create_meta_relationship(
    relationship_name, source, target, property_dir, metadata_path
):
    import os
    from os.path import join
    import re
